  ...
  describe 'methods' do
    ...
    describe '#voice_otp' do
      it 'fails if twilio validator fails' do
        ...
        expect(dummy_class).to receive(:render).with(xml: '<?xml version="1.0" encoding="UTF-8"?><Response><Hangup/></Response>', status: :unauthorized)
      ...
      it 'renders proper Twiml XML when validated' do
        ...
        expect(dummy_class).to receive(:render).with(
          xml: '<?xml version="1.0" encoding="UTF-8"?><Response><Say voice="alice">Hello! Your authorization code is 0,,0,,0. Once again, your authorization code is 0,,0,,0. Good bye!</Say></Response>'
        )
