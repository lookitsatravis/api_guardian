  ...
  module Helpers
    class Digits
      ...
      def authorize!
        ...
        fail ApiGuardian::Errors::IdentityAuthorizationFailed, "Digits API responded with #{response.code}. Expected 200!" unless response.code == 200
        response
      end
      ...
      def validate_oauth_key
        if ApiGuardian.configuration.registration.digits_key.blank?
          fail StandardError, 'Digits consumer key not set! Please add "config.registration.digits_key" to the ApiGuardian initializer!'
        end
      end
